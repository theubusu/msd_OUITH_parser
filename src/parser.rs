...
pub fn parse_ouith_blob(blob: &[u8]) -> Result<(Vec<MSDItem>, Option<OUSWImageVersionExDesc>), Box<dyn std::error::Error>> {
    ...
    while reader.stream_position()? < blob.len() as u64 {
        ...
        if top_descriptor.tag == 0x01 {
            ...
            println!("  OUDestinationDesc(0x03) - Size: {}", destination_descriptor.size);
            let out_size: u32 = reader.read_be()?;
            println!("      Out data size: {}", out_size);
            ...
            println!("      Type descriptor(0x{:02x}) - Size: {}", type_descriptor.tag, type_descriptor.size);
            ...
            println!("  OUDataProcessingDesc(0x07) - Size: {}", data_processing_descriptor.size);
            let heading_size: u32 = reader.read_be()?;
            println!("      Heading size: {}", heading_size);
            ...
            while reader.stream_position()? < epos {
                ...
                if descriptor.tag == 0x0E {
                    //OUAESEncryptionDesc
                    println!("          OUAESEncryptionDesc(0x0E) - Size: {}", descriptor.size);
                    ...
                    //OURSAValidationDesc
                    println!("          OURSAValidationDesc(0x10) - Size: {}", descriptor.size);
                    ...
                    println!("          OUCRC32ValidationDesc(0x12) - Size: {}", descriptor.size);
                    let crc32: u32 = reader.read_be()?;
                    println!("              CRC32: {:02x}", crc32);
                    crc32_hash = Some(crc32);
                }
                else if descriptor.tag == 0x18 {
                    //OUSecureHashValidationDesc
                    println!("          OUSecureHashValidationDesc(0x18) - Size: {}", descriptor.size);
                    ...
                    println!("          Unimplemented descriptor(0x{:02x}) - Size: {}", descriptor.tag, descriptor.size);
            ...
            println!("  OUGroupInfoDesc(0x13) - Size: {}", group_info_descriptor.size);
            ...
            if version_descriptor.tag == 0x19 {
                println!("  OUSWImageVersionExDesc(0x12) - Size: {}", version_descriptor.size);
                ...
                //type not implemented,  ignore the data
                println!("  Unimplemented descriptor(0x{:02x}) - Size: {}", version_descriptor.tag, version_descriptor.size);
