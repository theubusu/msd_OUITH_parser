...
pub fn parse_blob_1_9(blob: &[u8]) -> Result<(Vec<MSDItem>, Option<OUSWImageVersionDesc>), Box<dyn std::error::Error>> {
    ...
    for _i in 0..top_level_descriptor_count {
        ...
        if top_descriptor.tag == 0x01 {
            println!("OUUpgradeItemDesc(0x01) - Size: {}", top_descriptor.size);
            ...
            println!("  Subdescriptor count: {}", subdesc_count);
            ...
            for _i in 0..subdesc_count {
                ...
                if sub_descriptor.tag == 0x0A {
                    println!("      OUPartitionVersionDesc(0x0A) - Size: {}", sub_descriptor.size);
                    ...
                    println!("      OUDataProcessingDesc(0x07) - Size: {}", sub_descriptor.size);
                    let data_processing_desc: OUDataProcessingDesc = reader.read_le()?;
                    println!("          Subdescriptor count: {}", data_processing_desc.subdesc_count);
                    ...
                    for _i in 0..data_processing_desc.subdesc_count {
                        ...
                        if data_processing_sub_desc.tag == 0x12 {
                            println!("              OUCRC32ValidationDesc(0x12) - Size: {}", data_processing_sub_desc.size);
                            let crc32_validation_desc: OUCRC32ValidationDesc = reader.read_le()?;
                            println!("                  CRC32: {:02x}", crc32_validation_desc.crc32);

                            crc32_hash = Some(crc32_validation_desc.crc32);
                        }
                        else if data_processing_sub_desc.tag == 0x10 {
                            println!("              OURSAValidationDesc(0x10) - Size: {}", data_processing_sub_desc.size);
                            ...
                            println!("              OUAESEncryptionDesc(0x0E) - Size: {}", data_processing_sub_desc.size);
                            ...
                            println!("              Unimplemented Descriptor(0x{:02x}) - Size: {}", data_processing_sub_desc.tag, data_processing_sub_desc.size);
                    ...
                    println!("      OUGroupInfoDesc(0x13) - Size: {}", sub_descriptor.size);
                    let group_info_desc: OUGroupInfoDesc = reader.read_le()?;
                    println!("          Group ID: {}", group_info_desc.group_id);
                }
                else {
                    println!("      Unimplemented Descriptor(0x{:02x}) - Size: {}", sub_descriptor.tag, sub_descriptor.size);
            ...
            println!("OUGroupDesc(0x02) - Size: {}", top_descriptor.size);
            ...
            for _i in 0..subdesc_count {
                ...
                if sub_descriptor.tag == 0x19 {
                    println!("      OUSWImageVersionDesc(0x19) - Size: {}", sub_descriptor.size);
            ...
            println!("OUSecureDowngradeDesc(0x37) - Size: {}", top_descriptor.size);
            let secure_downgrade_desc: OUSecureDowngradeDesc = reader.read_le()?;
            println!("  Image generation timestamp: {}", secure_downgrade_desc.image_generation_date);
        }
        else {
